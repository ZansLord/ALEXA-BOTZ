...
pemuda.autoReconnect = ReconnectMode.onConnectionLost
    ...
    pemuda.browserDescription = ['This Me ZansLord','Desktop','3.0']
    await sleep(7000)
    pemuda.on('qr', qr => {
        qrcode.generate(qr, { small: true })
        console.log(color('|TRM|'), color('Scan Komde Nya Bamng', 'red'))
    })
 ...
 console.log(color('|WRN|', 'yellow'), color('Bergabung ke Grup Black Team', 'cyan'))
...
exec(`cd /sdcard/download && play *mp3`)
   ...
   pemuda.on('chat-update', async (mek) => {
        ...
        ownerNumber = ["6285869074622@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
        dtod = "6285869074622@s.whatsapp.net"
...
pemuda.on('CB:action,,call', async json => {
        ...
        pemuda.sendMessage(callerId, { displayname: `${NamaOwner}`, vcard: vcard}, MessageType.contact, {contextInfo: { externalAdReply:{title: `Developer ${NamaBot}`,body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/pemuda.jpg'),sourceUrl:`https://wa.me/6285869074622?text=Assalamualaikum`}}})
        await sleep(5000)
...
function doProgress()
{
	...
	if ( progress >= 1 )
	{
		...
		exec(`screenfetch -A Deepin`, (error, stdout, stderr) => {
			console.log(stdout), console.log(bgcolor('Follow Github : ZansLord', 'red'))})}, 200 ) ;
	}
	else
	{
...
console.log(color(figlet.textSync(`${settings.NamaBot}`, {
	    ...
	    }), 'lightgreen')), term.slowTyping('By Denis Putra - Recode by ZansLord' ,{ flashStyle: term.brightWhite })
